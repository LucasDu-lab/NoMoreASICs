...
const char * const BITCOIN_CONF_FILENAME = "nomoreasics.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\NoMoreASICs
    //   new: C:\Users\Username\AppData\Local\NoMoreASICs
    // macOS: ~/Library/Application Support/NoMoreASICs
    // Unix-like: ~/.nomoreasics
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "NoMoreASICs";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "NoMoreASICs";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/NoMoreASICs";
#else
    // Unix-like
    return pathRet / ".nomoreasics";
#endif
#endif
}
